  ...
  chkStdCurSem(student_id, stud_name, function () {
    $.each(obj_stdCurSem.data, function (i, item) {
      ...
      let boxList =
        ...
        '"></textarea>' 
        +
        '<button   class="btn green-200 m-a btn_summary m-t" onclick="viewExamSlip(\'' + student_id + "','" + stud_name1 + "','" + item.fk_acaCal + "','" + item.cur_year +"','" +item.cal_cohort + "','" + stud_icno + "','" +  stud_cur_intake + "','" + stud_pgm_name +   '\',)"' + '><i class="fa fa-calendar"></i> Examination Slip</button>'
        +
        '<table id="crList_' +
        item.fk_acaCal +
      ...
      listByActPolicy(student_id, item.fk_acaCal, function () {
        ...
        if (itemJ.est_tableno != null && itemJ.est_tableno != ""){
           statusexam = '<span style="color: blue; font-weight: bold;">' + 'Sitting For Exam' + '</span>';
        }
        else{
...
$("#printTimeTableExam").on("click", function (e) {
  window.open("print_timetblSExam.html");
});
...
function listByActPolicy(std, acaCal, returnValue) {
  ...
  var settings = {
    url: host + "api_pengurusan_pelajar/public/misStdSlipExam/listBySessionSExam",
    method: "POST",
    timeout: 0,
    headers: {
      Authorization: "picoms " + window.sessionStorage.token,
