...
$(function () {
    ...
    student_info(student_id, function () {
        ...
        $('#yrsOfstudy').html(dataStd.pgm_duration);
        ...
        getPgmDet(dataStd.pgm_fk, session, dataStd.cur_intake, function () {
            ...
            auditCourse(obj_getId.dtp_id, student_id, function () {
                ...
                console.log(obj_cotDetList);

                $.each(obj_cotDetList.dataExpectedCrs, function (i, item) {
                    ...
                    let count = obj_cotDetList.dataStdCurr.length;

                    dataTaken = (count != 0) ? obj_cotDetList.dataStdCurr : '';

                    dataTakenFound = (obj_cotDetList.dataStdCurr).find(element=>element.crs_code === item.pk_id);

                    dataTakenFound ? dataTakenFound : '';
                    // dataTakenFound = dataTaken.find((element) =>  element.crs_code == item.ccd_id);
                    // console.log(dataTakenFound.grade);

                    // if (count != 0) {
                    //     $.each(obj_cotDetList.dataStdCurr, function (j, itemJ) {
                    //         rs_status = itemJ.rsb_status;
                    //         // rs_curYear = itemJ.cur_year;
                    //         // console.log(itemJ);
                    //     });
                    ...
                    // console.log(dataTakenFound);
                    // dataTakenFound.grade
                    let cur_year = '';

                    if (dataTakenFound !== undefined && dataTakenFound.grade != null && dataTakenFound.grade !== '') {
                        label = `<span class="">`+dataTakenFound.grade+`</span>`;
                        ...
                        if (dataTakenFound.cur_year) {
                            cur_year += dataTakenFound.cur_year.replace('/', '');
                        }
                        cur_year += `/`;
                        cur_year += dataTakenFound.cal_cohort || '';
                    } else {
                        label = '<span class="label primary text-uppercase">Register</span>';
                    }

                    if (dataTakenFound !== undefined && dataTakenFound.taken_semester != null && dataTakenFound.taken_semester !== '') {
                        labelTakenSem = dataTakenFound.taken_semester;
                    ...
                    list_data.push({
                        bil: bil++,
                        code_subject: '<span class="text-uppercase">' + item.crs_code + '</span>',
                        crs_name: '<span class="text-uppercase">' + item.crs_name + '</span>',
                        cot_semester: item.expected_sem,
                        taken_semester : labelTakenSem,
...
function auditCourseStdSite(id, student_id, returnValue) {
    var settings = {
        "url": host + "api_tetapan_picoms/public/misPrmCrsCOTDet/auditCourseStdSite/" + id + '/' + student_id,
        "method": "GET",
        "timeout": 0,
        "headers": {
            "Authorization": "picoms " + window.sessionStorage.token
    ...
    $.ajax(settings).done(function (response) {
        obj_cotDetListStdSite = response;
        returnValue();
    });
}
