$(function(){
    ...
    showProgram(idProgram, function(){
        ...
        if(!(progProposal == '' || progProposal == null)){
            progProposal = '<a target="_blank" style="color:cornflowerblue" href="'+host+'api_hep/public/proposal/'+progProposal+'"><span class="label info">Document</span></a>';
        }

        if(!(progReport == '' || progReport == null)){
            progReport = '<a target="_blank" style="color:cornflowerblue" href="'+host+'api_hep/public/progAct_report/'+progReport+'"><span class="label info">Document</span></a>';
        }

        if(!(progCert == '' || progCert == null)){
            progCert = '<a target="_blank" style="color:cornflowerblue" href="'+host+'api_hep/public/progAct_cert/'+progCert+'"><span class="label info">Document</span></a>';
        }
        
        $('#prog_peruntukan').html(data.prog_peruntukan);
    $('#prog_alloc_approve').html(data.prog_alloc_approve);
        ...
        $('#view_prog_startdate').html(formatDate(data.prog_startdate)+' - '+formatDate(data.prog_enddate));
...
$('#btnMdlUpdate').click(function(){ 
    ...
    showProgram(id, function(){
        ...
        $('#prog_status').val(data.prog_status);
        $('#prog_statusremark').val(data.prog_statusremark);
        $('#upt_prog_alloc_approve').val(data.prog_alloc_approve);

        $('#upt_prog_cost').val(data.prog_cost);
        $('#upt_prog_peruntukan').val(data.prog_peruntukan);
        $('#upt_prog_participate').val(data.prog_participate);
...
$('#formUptReport').on('submit', function(e){
    if(!confirmed){
        ...
        swal({
            ...
            var settings = {
                "url": host+"api_hep/public/hepProgram/uptRprtByStd",
                "method": "POST",
                "timeout": 0,
                "headers": {
                    "Authorization": "picoms " + window.sessionStorage.token
...
function btn_cert(id){
    ...
    window.sessionStorage.imgUrl = $('#imgUrl').val();
