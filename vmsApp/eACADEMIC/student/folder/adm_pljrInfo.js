function checkSize() {
    ...
    if (typeof (fileUpload.files) != "undefined") {
        ...
        // alert(size);
        if(size > 20000){
            $('#checkSize').html('Profile image size must be <strong>smaller than 1000KB</strong>.');
...
var viewProgram = function () {
    ...
    $.ajax(settings).done(function (response) {
        ...
        // $('#upt_programmeSelected').append($('<option>', { 
        //     value: response.data.pgm_id,
        //     text : response.data.pgm_name 
        // }));
        $.each(response.data, function (i, item) {
            $('#upt_programmeSelected').append($('<option>', { 
                value: item.pgm_id,
...
function loadData(indexs){   
    ...
    // console.log(data[indexs].pgm_id)
    ...
    if(receipt == ''){
        ...
        $('#upt_srg_payment_resitdoc_download').attr("href", '../api_pengurusan_pelajar/public/'+data[indexs].srg_payment_resitdoc);
    }
    ...
    if(certificate == ''){
        ...
        $('#upt_sta_cert_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+certificate);
    }
    ...
    if(spm == ''){
        ...
        $('#upt_sta_spm_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+spm);
    }
    ...
    if(stpm == ''){
        ...
        $('#upt_sta_stpm_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+stpm);
    }
    ...
    if(diploma == ''){
        ...
        $('#upt_sta_diploma_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+diploma);
    }
    ...
    if(degree == ''){
        ...
        $('#upt_sta_degree_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+degree);
    }
    $('#upt_sta_degree_doc_download').attr("href", '../api_pengurusan_pelajar/public/'+data[indexs].sta_degree_doc);
...
var RegisterModel = function () {
   ...
   //     required: true
   // });
    self.sts_date_complete = ko.observable("");
    ...
    self.Register = function () {
        ...
        let upload_0_size = upload_0.size / 1024;
        if (upload_0_size > 10000) {

            swal(
                'Student Information',
                'Profile picture size is too big.',
                'error'
              )
            return;
        }
