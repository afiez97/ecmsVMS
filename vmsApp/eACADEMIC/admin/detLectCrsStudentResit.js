...
$(function(){
    ...
    findId(crsId, function(){
        ...
        $.each(obj_grdSchm.data, function(j, itemJ){
            ...
            gradeCmpnnt(gscId, function(){
                ...
                $.each(obj_grdSchmDet.data, function(i, item){
                    ...
                    if(item.gsd_component == "Continuous Assessment"){
                        ...
                        if((i+1) == obj_grdSchmDet.data.length){
                            column_cMark = true;
                        }
                    }
            ...
            subItemList(id, fk_cotDet, function(){
                ...
                $.each(obj_subItem.data, function(i, item){
                    ...
                    // columns.push({"name":item.item_name,"title":item.item_name+' ('+item.non_obe_percentage+'%)',"breakpoints":"md sm xs"});
        ...
        // student list
        stdByAcaCalCrs3(fk_aca_cal, crsId, function(){
            // console.log(fk_aca_cal + '-'+ crsId);
            columns.push(
            ...
            // console.log(obj_stdRegCrs.data);
            ...
            $.each(data_student, function (j, itemJ){
                ...
                if(yearTaken.replace('/','') >= "20222023"){
                    if(cal_cohort < "3" && yearTaken.replace('/','') == "20222023"){
                        ...
                        stdAttend = stdAttend.filter(record => record.fk_exam_type === 4 || record.fk_exam_type === 5);
                        
                        if(stdAttend.length >= 1){
                  

                            latestEntry = stdAttend.reduce((latest, current) => {
                                return (latest.pk_id > current.pk_id) ? latest : current;
                            });
                            // if(stdAttend[0].attendance == "Attend"){
                            if(latestEntry.attendance == "Attend"){
                                tMark = (itemJ.tMark)*1;
                                grade = itemJ.grade;
                                point = itemJ.point;
                                action_btn = '';
                ...
                $.each(list_gradeCmpnnt,function(f,item){
                    ...
                    if(names != item.gsd_id){
                            ...
                            if(item.gsd_component == "Continuous Assessment"){
                                ...
                                if(yearTaken.replace('/','') >= "20222023"){
                                    if(cal_cohort < "3" && yearTaken.replace('/','') == "20222023"){
                                        ...
                                        stdAttend = filterData(dataAttendance, filterAttendance);
                                        //  lastElement = stdAttend[stdAttend.length - 1];
                                        stdAttend = stdAttend.filter(record => record.fk_exam_type === 4 || record.fk_exam_type === 5);
                                         lastElement = stdAttend[stdAttend.length - 1];
                                        ...
                                        if(stdAttend.length > 0){
                                            if (itemJ.app_type != null) {
                                         typeLabel = '('+itemJ.app_type+')';
                                                
                                            } 

                                            attend = `<br><span class="label danger">Not Attend`+typeLabel+`</span>`;
                                            // if(stdAttend[0].attendance === "Attend"){
                                            if(lastElement.attendance === "Attend"){
                                                // console.log(lastElement)

                                                attend = `<br><span class="label success">Attend <br>`+typeLabel+`</span>`;                                    
                                            }
                        ...
                        viewData["mark"+item.gsd_id] = `<p class="text-info text-center"><b id="`+itemJ.std_id+'_'+item.gsd_id+`">`+val+`</b>`+attend+`</p>`;
                    }
                });              
                            ...
                            await listSubItem.forEach(async (item,f) => {
                                ...
                                if(results.success){   
                                    ...
                                    if(names != itemJ.std_id + '_' + dataMark[0].fk_gsDet){
                                        if(f != 0){
                                            if(itemJ.std_id == "BSC09220001"){
                                                // console.log(itemJ.std_id+' : '+marks + '/' + full_mark + '*' + '100*(' + percentage + '/' + '*100)');
                                            }
                                        ...
                                        if((f+1) == listSubItem.length){
                                            ...
                                            // if(itemJ.attendance != "Attend"){
                                            //     tMark = 0.00;                                   
                                            // }
                                            

                                           if (tMark > 59 && itemJ.app_type === "RE-SIT"){
                                                console.log(tMark);
                                            ...
                                            form.append("marks", tMarkSpecial.toFixed(0));
                                            ...
                                            if(obj_grd.success){
                                                ...
                                                checkingBtn() // pakai lpas tekan show dia akan check
                                            }        
                                        }
                                    }
                                    else{
                    ...
                    async function main() {
                        ...
                        await Promise.all(
                            objStd.map(async (itemJ,i) => {
                                ...
                                if(list.length > 0){
                                    await doSomethingAsync('<tr><td>'+itemJ.std_id+'</td><td>'+itemJ.sti_name+'</td><td>'+itemJ.cMark+'</td><td>'+itemJ.tMark+'</td>'+
                                    '<td>'+itemJ.grade+'</td><td>'+itemJ.point+'</td></tr>');
                                    // list = [];
                                }
        ...
        await Promise.all(
            dataStd.map(async (item,i) =>{
                ...
                if(!obj_stdMark.success){
                    ...
                    // if (!result.success) {
                    //     swal("Register Student Mark Failed", stdId, "error");
                    //     return;
                    // }
...
function filterData(data, conditions) {
  ...
  function stdByAcaCalCrs3(acaCal, crs, returnValue) {
    var form = new FormData();
    form.append("aca_session", acaCal);
    form.append("crs_code", crs);
  
    var settings = {
      url: host + "api_pengurusan_pelajar/public/misStdRegsub/listByAcaCalCrs3",
      method: "POST",
      timeout: 0,
      headers: {
        Authorization: "picoms " + window.sessionStorage.token,
    ...
    $.ajax(settings).done(function (response) {
      obj_stdRegCrs = JSON.parse(response);
      // Filter out entries with barr_status = 'Barred'
      obj_stdRegCrs.data = obj_stdRegCrs.data.filter(function (entry) {
        // return entry.barr_status !== 'Barred';
     
        return (entry.barr_status !== 'Barred' && entry.app_status == 'Completed');
    });
  returnValue();
    });
  }
