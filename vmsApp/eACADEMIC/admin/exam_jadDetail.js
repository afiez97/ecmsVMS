$(function () {
        ...
        if (timetblData.length == 1) {
            $.each(timetblData, function (i, item) {
                ...
                listTblNo(timetblId, function () {
                    ...
                    $('#totalStudent').val(dataLength);
                    // console.log(timetblId);
                    // console.log(obj_tblNoList.data);
                    if (dataLength != 0) {
                        let hide = '';
    ...
    stdByAcaCalCrs_jadDetail(aca_cal, fk_course, function () {
        ...
        $.each(obj_stdRegCrs.data, function (j, itemJ) {
            ...
            list.push({
                ...
                chkbox: '<label class="ui-switch primary m-t-xs m-r" '+ExamUpdateHidden+' ><input type="checkbox" id="chkbox-' + j + '" onclick="chkBox(\'' + j + '\',\'' + itemJ.std_id + '\')" ' + chk + '><i></i></label>'
            });
...
function chkBox(indexs) {
    ...
    else { postStatus('Barred', data.rsb_id); }
}
//-------------------------------------------------- end change status --------------------------------------------------//
...
function saveStdTblNo(getData) {
    ...
    if (getData.fk_examApplication == null) {
        afiez = '';
    } else {
        afiez = getData.fk_examApplication;
    ...
    form.append("fk_exam_application", afiez);
    ...
    swal({
        ...
        $.when(
            $.each(data, function (i, item) {
                ...
                //     }
                // });
                if (window.sessionStorage.typeExam === 'FINAL EXAMINATION ') {
...
//                 // nie start utk check final exam 
//                 if (window.sessionStorage.typeExam === 'FINAL EXAMINATION ') {
//                     obj = new post(host + 'api_exam_picoms/public/misExamStd/uptTblNo', form, 'picoms ' + window.sessionStorage.token).execute();
//                     if (obj.success) {
//                         swal({
//                             text: "Finished",
//                             type: "info"
...
//                 }
//                 // nie end utk check final exam 
//                 // nie start utk check slain final 
//                 else {
...
//                         var form_updCompleted = new FormData();
//                         form_updCompleted.append("pk_id", item.fk_applicationExam);
//                         obj_updCompleted = new post(host + 'api_exam_picoms/public/misExamApp/updCompleted', form_updCompleted, 'picoms ' + window.sessionStorage.token).execute();
//                         if (obj_updCompleted.success) {
...
async function uptExmStd(index, totalCount) {
    // console.log("Total count:", totalCount);
    let dtStd = JSON.parse($('#tblNoList').val());

    $("#loading_modal").modal('show');

    // Find the specific item that matches index.no
    let item = dtStd.find((item, i) => index.no === i);

                // Calculate the percentage of completed updates
                let totalStudents = $('#totalStudent').val();

    if (item) {
        completedCount++;
        let percentage = (100 * completedCount) / totalStudents;
        ...
        try {
            let url = host + 'api_exam_picoms/public/misExamStd/uptTblNo';
            let headers = {
                "Authorization": "picoms " + window.sessionStorage.token
            };
            ...
            if (response.success) {
                if (window.sessionStorage.typeExam === 'FINAL EXAMINATION ') {
                    ...
                    // Logic for non-final examination
                    let form_updCompleted = new FormData();
                    form_updCompleted.append("pk_id", response.data.fk_applicationExam);

                    let completedUrl = host + 'api_exam_picoms/public/misExamApp/updCompleted';
                    let completedResponse = await fetch(completedUrl, {
                        method: 'POST',
                        body: form_updCompleted,
                        headers: headers
                    }).then(res => res.json());
                    ...
                    if (completedResponse.success) {
                        ...
                        Swal(completedResponse.message, completedResponse.data, "error");
                        return;
                    }
                }
                // console.log(completedCount +'-'+ $('#totalStudent').val()  );

                // $('#totalStudent').val() <= (dtObjects.length)
    ...
    window.location.reload();
                    ...
                    console.log("Pending updates detected.");
                }
            ...
            Swal("Error during processing", error.message, "error");
        }
    }
}
...
//         let updateCompletedPromises = responses.map(obj => {
//             if (obj.success) {
//                 if (window.sessionStorage.typeExam !== 'FINAL EXAMINATION ') {
//                     // If not final exam, update completed status
//                     let form_updCompleted = new FormData();
//                     form_updCompleted.append("pk_id", obj.data.fk_applicationExam);

//                     return fetch(host + 'api_exam_picoms/public/misExamApp/updCompleted', {
//                         method: 'POST',
//                         body: form_updCompleted,
//                         headers: {
//                             "Authorization": "picoms " + window.sessionStorage.token
...
//                     }).then(response => response.json());
//                 } else {
//                     // Return resolved promise if no further update is needed
//                     return Promise.resolve({ success: true });
//                 }
//             } else {
//                 return Promise.reject(new Error(obj.message));
//             }
//         });

//         let completedResponses = await Promise.all(updateCompletedPromises);

//         // Notify user based on responses
//         completedResponses.forEach(obj_updCompleted => {
//             if (obj_updCompleted.success) {
//                 swal({
//                     text: "Finished",
//                     type: "info"
//                 });
//             } else {
//                 Swal(obj_updCompleted.message, obj_updCompleted.data, "error");
//             }
//         });

//         // Check if all updates are complete
//         // `requests.length` is the total number of requests made to update table numbers
    ...
    swal({
        ...
        $.each(data, function (j, item) {
            ...
            chkVenueExist(chkVenue, function () {
                ...
                // console.log(AvailaibleCapacity)
                ...
                // Run uptExmStd(dt) for each dt in dtObjects
                if($('#totalStudent').val() <= (dtObjects.length)){
                    // console.log(    $('#totalStudent').val() +' - '+dtObjects.length);
                    dtObjects.forEach(async (dt) => {
                    await uptExmStd(dt, dtObjects.length);
                });
                }
            
                // start sni coding lama
...
function stdByAcaCalCrs_jadDetail(acaCal, crs, returnValue) {
    var form = new FormData();
    ...
    $.ajax(settings).done(function (response) {
        ...
        // sessionExamApi == sessionExam
        // console.log(obj_stdRegCrs);

        obj_stdRegCrs.data = obj_stdRegCrs.data.filter(function (entry) {
            // return entry.barr_status !== 'Barred';

            // if (window.sessionStorage.typeExam === 'FINAL EXAMINATION') {
            //     sessionExam = null;
            // } else {
            //     sessionExam = (window.sessionStorage.typeExam).split(' ')[0];
            // }

            // if (entry.app_type === null) {
            //     sessionExamApi = null;
            // } else {
            //     sessionExamApi = (entry.app_type).split(' ')[0];
            // }


            let sessionExam = window.sessionStorage.typeExam.split(' ')[0];
            // let sessionExam = window.sessionStorage.typeExam === 'FINAL EXAMINATION' ? 'FINAL EXAMINATION' : window.sessionStorage.typeExam.split(' ')[0];
            sessionExamApi = entry.app_type === null ? 'FINAL' : entry.app_type.split(' ')[0];


            // return sessionExamApi === sessionExam;
            return (sessionExamApi === sessionExam) && entry.app_status != 'Reject' && entry.recExamApplication !== 'DEL';
        });
