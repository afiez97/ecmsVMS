$(function () {
    ...
    async function main(){
        ...
        if(obj.success){
            let data = obj.data;
            console.log(data)
            let jobGrade = data.employee.main_position.job_grade.code;
            let access_idStaff = ["DS41", "DS45", "DS51", "DS52", "DS53", "DS4"].includes(jobGrade) ? 4 : null;
            ...
            //     Emp_Id:data.employee.staff_id,
            //     Job_Code:data.employee.main_position.job_grade.code,
            //     Post_Id:"000",
            ...
            race = data.employee.race === 'MALAY' ? 'Melayu' : data.employee.race;
            ...
            if (objUsers.success) {
                ...
                let checkingInfoHRMTable = await new getPublic(host + 'api_hr_emp/public/employee/view/' + data.employee.staff_id, token_private).execute();
                if (checkingInfoHRMTable.success) {
                    ...
                    console.log(hrm_emp_info_ha);
                    let updatedEMPInfo = await new postPublic(host + 'api_hr_emp/public/employee/update', JSON.stringify(hrm_emp_info_ha), token_private).execute();
                    let result = await new postPublic(host + 'api_hr_emp/public/users/login', form, token_private).execute();
                    ...
                    if (result.success) {
                        ...
                        window.sessionStorage.from = "SSO";
                        window.location.replace("campusPage.html");
                    }
                    ...
                    let objEmpInfo = await new postJson(host + 'api_hr_emp/public/employee/create', hrm_emp_info_ha, token_private).execute();
                    if (objEmpInfo.success) {
                ...
                // console.log(objEmpInfo);
                // console.log(objEmpPostion);
